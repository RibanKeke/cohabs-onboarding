...
describe("Report progress by type", () => {
  ...
  test("Should display info progress with data", () => {
    ...
    ReportAgent.logProgress<TestData>("Test", "Testing Progress", "info", {
      data: [testData],
      reportFields: ["name", "address"],
    });
    const report = ReportAgent.getReport().find((report) =>
  ...
  test("Should display failure progress", () => {
    ...
    ReportAgent.logProgress<TestData>("Test", "Testing Progress", "failure", {
      data: [testData],
      reportFields: ["name", "address"],
    });
    const report = ReportAgent.getReport().find((report) =>
      report.includes("failure")
    );
    expect(report).toBeDefined();
    ReportAgent.clearReport();
  });
  ...
  test("Should display failure progress with data", () => {
    ...
    ReportAgent.logProgress<TestData>("Test", "Testing Progress", "failure", {
      data: [testData, { ...testData, name: "Stephane" }],
      reportFields: ["name", "address"],
    });
    const report = ReportAgent.getReport().find((report) =>
      report.includes("Robert")
    );
    const report_2 = ReportAgent.getReport().find((report) =>
      report.includes("Stephane")
    );
    expect(report).toBeDefined();
