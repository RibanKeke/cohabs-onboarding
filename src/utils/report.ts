...
class ReportAgent {
	static logProgress<T>(title: string, description: string, type: ReportType, payload?: Payload<T>) {
		if (!this.instance) {
	...
	private printData<T>(data: T[], reportFields: Array<keyof T>) {
		const printedData = data.map(dataItem => {
			const filteredData = reportFields.reduce<Partial<T>>((filtered, field) => {
				const value: unknown = (dataItem as Record<string, any>)[String(field)];
	...
	private reportProgress<T>(title: string, description: string, type: ReportType, payload?: Payload<T>) {
		...
		this.reports.push(`[${type}] - [${title}: ${description}] \n ${dataReport}`);
	}
}
