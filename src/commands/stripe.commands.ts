...
async function syncUsers(): Promise<ExecutionSummary> {
  ...
  const missingUsers = Object.values(usersSummary.missing).map(
    (executionRecord) => executionRecord.user
  );
  Report.logProgress<Entities.Users>(
    "...Progress:",
    "Missing stripe users",
    "warning",
    {
      data: missingUsers,
      reportFields: [
  ...
  const invalidStipeUsers = Object.values(usersSummary.invalid).map(
    (executionRecord) => executionRecord.user
  );
  Report.logProgress<Entities.Users>(
    "...Progress:",
    "Invalid stripe users",
    "warning",
    {
      data: invalidStipeUsers,
      reportFields: [
        "active",
        "id",
        "firstName",
